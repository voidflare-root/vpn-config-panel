...
const DEFAULT_CONFIG = JSON.stringify(
  { version: "1.0.0", config: "Paste your VPN config here" },
  null,
  2
);
...
async function handleRequest(req, res) {
  ...
  if (url.pathname === "/api/config" && req.method === "GET") {
    ...
    return sendJson(res, 200, {
      config,
      path: useGithubStorage() ? GITHUB_CONFIG_PATH : CONFIG_PATH,
  ...
  if (url.pathname === "/api/config" && req.method === "PUT") {
    ...
    try {
      normalizedConfig = JSON.stringify(JSON.parse(config), null, 2);
    } catch {
      return sendJson(res, 400, { error: "Config must be valid JSON for data/config.json" });
    }
...
ensureConfigFile().then(() => {
  http
    ...
    .listen(PORT, () => {
      ...
      console.log(
        `Raw config link: ${useGithubStorage() ? githubRawUrl() : `http://localhost:${PORT}/raw/config`}`
      );
    });
});
